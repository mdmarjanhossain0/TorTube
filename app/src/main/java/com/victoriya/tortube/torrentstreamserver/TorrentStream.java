...
public final class TorrentStream {
    ...
    private static final String
            LIBTORRENT_THREAD_NAME = "TORRENTSTREAM_LIBTORRENT",
            STREAMING_THREAD_NAME = "TORRENTSTREAMER_STREAMING";
    ...
    private TorrentInfo getTorrentInfo(String torrentUrl) throws TorrentInfoException {
        if (torrentUrl.startsWith("magnet")) {
        ...
        }else if (torrentUrl.startsWith("content")) {
            Uri path = Uri.parse(torrentUrl);
            /*File file = new File(path.getPath());
            ...
            try (ParcelFileDescriptor outPfd = contentResolver.openFileDescriptor(path, "r")) {
                if (outPfd == null) {
                ...
                try (FileInputStream is = new FileInputStream(outFd)) {
                    ...
                    byte[] responseByteArray = getBytesFromInputStream(is);
                    is.close();
